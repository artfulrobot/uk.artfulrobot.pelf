...
function _civicrm_api3_activity_Getpelfcontract_spec(&$spec) {
  ...
  $spec['id']['description'] = ts('The Activity ID of this contract');
...
function civicrm_api3_activity_Getpelfcontract($params) {
  ...
  $_['return'] = "id,activity_type_id,subject,activity_date_time,details";
  if ($activity['activity_type_id'] != $pelf->getContractActivityType()) {
    throw new API_Exception("Not a contract");
  }
  ...
  if (!empty($params['with_activities'])) {
    // Look up activities for these contacts, since the contract's date.
    $query_params = [];
    $i=1;
    $sql_contact_ids = [];
